...
namespace MAP_INP
{
    void initUndistortRectifyMap(parameters &params, cv::Mat &map1, cv::Mat &map2)
    {
        ...
        buildInterpolatedAzimuthModel(params.objpoints, params.imgpoints, inp_model);
    ...
    void buildInterpolatedAzimuthModel(const vector<vector<cv::Point2f>> &objpts, const vector<vector<cv::Point2f>> &imgpts, vector<MAP_INP::remapIp> &inp_model)
    {
    ...
    void saveCalibrationMapToYAMLFile(const string &file_folder, const parameters &params)
    {
        if (!doesFolderExists(file_folder))
        {
            cout << "fisheye camera calibration file:" << file_folder << " DOESN'T EXISTS. Creating it" << endl;
            mkdir(file_folder.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        }

        // ROS_INFO("Writing calibration map YAML file to %s", calibrationMapFile.c_str());
        FILE *yaml = fopen((file_folder + "/intrinsic_calibration.yaml").c_str(), "w");

        fprintf(yaml, "img_size: [%d, %d]\n", params.imgSize.width, params.imgSize.height);
        fprintf(yaml, "calib_dist_mm: [%.2f, %.2f]\n", params.horizCalDist, params.vertCalDist);

        fprintf(yaml, "cal_pts:\n");
        for (int j = 0; j < params.objpoints.size(); j++)
        {
            fprintf(yaml, "- row:\n");
            for (int i = 0; i < params.objpoints[j].size(); i++)
            {
                fprintf(yaml, "  - obj: [%d, %d]\n", int(params.objpoints[j][i].x), int(params.objpoints[j][i].y));
                fprintf(yaml, "    img: [%d, %d]\n", int(params.imgpoints[j][i].x), int(params.imgpoints[j][i].y));
            }
        }
    ...
    bool saveCalibrationMapsBinary(const string &path, const cv::Mat &mapx, const cv::Mat &mapy)
    {
        ...
        {
            cout << "saga_fisheyecam_calibration:" << path << " DOESN'T EXISTS. Creating it" << endl;
    ...
    {
        cout << "saga_fisheyecam_calibration: opening camera calibration map YAML file:" << path + "/intrinsic_calibration.yaml" << endl;
        ...
        vector<double> calib_dist_mm = doc["calib_dist_mm"].as<vector<double>>();
        cout << "calib_dist_mm: horiz:" << calib_dist_mm[0] << " vert:" << calib_dist_mm[1] << endl;
        params.horizCalDist = double(calib_dist_mm[0]);
        params.vertCalDist = double(calib_dist_mm[1]);
